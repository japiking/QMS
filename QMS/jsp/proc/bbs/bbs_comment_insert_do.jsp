...
public class bbs_comment_insert_do implements SpbBiz {
    ...
    public Map execute(HttpSession session, HttpServletRequest request, Map trmap) throws Exception{
    	...
    	String boardID 					= StringUtil.null2void((String)tranReqMap.get("BOARD_ID"));    	
    	...
    	LogUtil.getInstance().debug("userId:: "+ userId);
    	LogUtil.getInstance().debug("boardID:: "+ boardID);
    	LogUtil.getInstance().debug("bbsID:: "+ bbsID);
    	LogUtil.getInstance().debug("contents:: "+ contents);
    	...
    	try{
    		...
    		try {
    			...
    			result = qmsDB.insert("QMS_BBS_DETAIL.BBS_COMMENT_C001", paramC001);
    		} catch (Exception e) {
...
%>
<%
	request.setAttribute(SpbBiz.class.getName(), new bbs_comment_insert_do());
%>