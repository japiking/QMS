...
<%!public class test_update_do implements SpbBiz {
		...
		public Map execute(HttpSession session, HttpServletRequest request, Map trmap) throws Exception {
			...
			try {
				...
				try {
					...
					param.put("TEST_NM",		dataM.get("TEST_NM"));
					param.put("TEST_STTG_DATE",	dataM.get("TEST_STTG_DATE"));
					param.put("TEST_ENDG_DATE",	dataM.get("TEST_ENDG_DATE"));
					param.put("DEL_FLAG",		dataM.get("DEL_FLAG"));
					param.put("TEST_BIGO",		dataM.get("TEST_BIGO"));
					param.put("TESTER_ID",		userSession.getUserID());
					
					checkMap = qmsDB.selectOne("QMS_QUALITYCONTROL.TEST_MANAGEMENT_R002", param);
					
					String checkFlag = 	StringUtil.null2void(checkMap.get("DEL_FLAG"));
					String del_flag	 =  StringUtil.null2void(dataM.get("DEL_FLAG"));
					
					flag = qmsDB.update("QMS_QUALITYCONTROL.TEST_MANAGEMENT_U001", param);
					...
					if(!checkFlag.equals(del_flag)){
						...
						flag = qmsDB.update("QMS_QUALITYCONTROL.STEP_U001",param);					//역쇄 삭제 update
