...
public class defect_list_do implements SpbBiz {
    ...
    public Map execute(HttpSession session, HttpServletRequest request, Map trmap) throws Exception{
		...
		String userId							= StringUtil.null2void(tranReqMap.get("TEST_ID"));
    	...
    	try{
    		...
    		try {
    			...
    			if(!"".equals(userId)){
    				param.put("TEST_ID", 			userId);
    			}
    			param.put("TESTER_ID", 			userSession.getUserID());
    			param.put("DEVELOPER_ID", 		userSession.getUserID());
    			param.put("PROJECT_ID", 		userSession.getProjectID());
    			result = qmsDB.selectList("QMS_DEFECT.DEFECT_MANAGEMENT_R001", param);
    		} catch (Exception e) {
...
%>
<%
	request.setAttribute(SpbBiz.class.getName(), new defect_list_do());
%>