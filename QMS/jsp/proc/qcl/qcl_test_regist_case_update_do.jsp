...
<%!public class test_regist_case_update_do implements SpbBiz {
		...
		public Map execute(HttpSession session, HttpServletRequest request,
				Map trmap) throws Exception {
			...
			try {
				...
				try {
					...
					param.put("CASE_NM",		dataM.get("CASE_NM"));
					...
					checkMap = qmsDB.selectOne("QMS_QUALITYCONTROL.CIRCUMSTANCE_R002", param);
					...
					if(!checkFlag.equals(del_flag)){
						...
						flag = qmsDB.update("QMS_QUALITYCONTROL.STEP_U001",param);
						flag = qmsDB.update("QMS_QUALITYCONTROL.STEP_ACTION_D002",param);			//연쇄 삭제 update
...
<%
	request.setAttribute(SpbBiz.class.getName(),
			new test_regist_case_update_do());
%>