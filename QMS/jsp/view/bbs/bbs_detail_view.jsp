...
//	LogUtil.getInstance().debug("SAMGU detailpage boardID:" +boardID+ ", bbsID:"+ bbsID +", seq:"+ seq +", depth:"+ depth );
	...
	try {
		...
		LogUtil.getInstance().info("조회수 증가시 query error : " + e.toString()); 
	}
	...
	try {
		...
		list = qmsDB.selectList("QMS_BBS_DETAIL.BOARD_R001", paramR001);
		
		LogUtil.getInstance().info("SAMGU >>> value:" + list);
		
	} catch(Exception e) {
		if (qmsDB != null) try { qmsDB.close(); } catch (Exception e1) {}
		LogUtil.getInstance().info("게시글 조회 query error : " + e.toString()); 
	} 
%>
...
function uf_deleteComment(bbsID,commentID) {
	if (confirm("정말로 삭제하시겠습니까?") == true){
		...
		ajax.set("TASK_PACKAGE",	"bbs" );				// [필수]업무 package 호출할 페이지 패키지
		...
		ajax.execute(function(dat) {
			try{
				...
				if ( commentList.length==0){
					...
					var innerHtml = "<h4>댓글  "+commentCount+"건</h4>"+
					"<table class=\"mgT10\">"+
						"<colgroup>"+
							"<col width=\"10%\"/>"+
							"<col width=\"*\"/>"+
					...
					for (var i=0; i< commentCount; i++) { 
						...
						if(comment.COMMENT_USER == user_id || grade == "00"){	// 작성자가 자기자신이거나 관리자는 수정삭제가 가능하다.
							...
							/* innerHtml+= "		<a href=\"#FIT\" class=\"btn\" onclick=\"javascript:uf_deleteComment(\'"+comment.BBS_ID+"\',\'"+comment.COMMENT_ID+"\');\" ><span>삭제</span></a>"; */
							innerHtml+= "	</td>";
							innerHtml+= "</tr>";
						} else {
							innerHtml+=	"<tr><th scope=\"row\">"+comment.USERNAME+"</th>";			
		...
		<% } else {
			...
			if ("Y".equals(dataMap.get("BBS_FILE"))) {
				...
				try {
					...
					paramR003.put("BBS_ID", 	bbsID);
					attach_list = qmsDB.selectList("QMS_BBS_DETAIL.BOARD_R003", paramR003);
				} catch (Exception e) {
					if (qmsDB != null) try { qmsDB.close(); } catch (Exception e1) {}
				...
				if("완료".equals(state) || "제외".equals(state)){
			%>
			...
			<table class="mgT10">
				...
				<tbody>
					...
					<tr>	
						...
						<th scope="row">확인자</th>			
						<td><%=StringUtil.null2void(dataMap.get("CONFIRM_NM"))%>(<%=StringUtil.null2void(dataMap.get("CONFIRM_USER"))%>)</td>								
					</tr>
					<tr>
						<th scope="row">내용</th>			
						<td colspan="3" style="height: 250px; width: 400px"><%=dataMap.get("CONTENTS")%></td>
					</tr>
					<tr>
						<th scope="row">첨부파일</th>
						<td colspan="3" class="wBtn">
							<% 
