...
try {
	...
	paramR001.put("RECIPIENT_ID",	userId); 
	paramR001.put("FROMCNT",		String.valueOf(fromcnt));
	paramR001.put("TOCNT",			String.valueOf(tocnt));
	
	list = qmsDB.selectList("QMS_BBS_ONELOW.BOARD_R001", paramR001);
...
try {
	...
	countMap	= qmsDB.selectOne("QMS_BBS_ONELOW.BOARD_R002", paramR002);
	
} catch (Exception e) {
	if (qmsDB != null) try { qmsDB.close(); } catch (Exception e1) {}
	LogUtil.getInstance().info("한줄게시판 조회  query error : " + e.toString()); 
}
...
function uf_inq(pagenum) {	// 파라메터 : 조회 페이지
	...
	form.target			= "_self";
	form.action			= "/QMS/jsp/view/bbs/bbs_oneRowList_view.jsp";
	form.submit();
}
				...
				<tr>
					...
					<%
						try {
							...
							sublist = qmsDB.selectList("QMS_BBS_ONELOW.BBS_RECIPIENT_R003", paramR003);
					...
					<td>
					<%	if ("S".equals(dataMap.get("RECIPIENT_DVD")) || "00".equals(userSession.getAuthorityGrade())) { // 보낸메시지일 경우 수정할수 있다. 관리자도 수정 가능하다.%>
